...
public class PlayerController : MonoBehaviour
{
    ...
    private void SetSpineConstraintsOffset(bool clipped)
    {
        ...
        targetOffsetY = _holstered ? 0f : targetOffsetY;
    ...
    public void HideGrenadeIndicators(Rigidbody rb)
    {
    ...
    public void ThrowGrenade(Rigidbody rb)
    {
        Rigidbody sRB = Instantiate(rb.gameObject).GetComponent<Rigidbody>();

        sRB.velocity = Vector3.zero;
        sRB.angularVelocity = Vector3.zero;

        sRB.isKinematic = false;
        sRB.transform.SetParent(null);

        sRB.AddForce(ReleasePosition.forward * ThrowStrength, ForceMode.Impulse);
        sRB.GetComponent<GrenadeItem>().ExplodeGrenade();
    }
...
/* 
 * (DONE) Stop Rotating As well when holstered.
 * (DONE) Check if weapon is not gun shot timer also.
 * (DONE) Allow different weight settings foreach constraint and choose if spine rotation offset should be applied.
 * and if so then give a 0, 1 slider to choose weight of the offset.
 * 
 * (DONE) Combine Repeating Functions Into Small quick functions.
 */

// Great 1708 lines of code already...